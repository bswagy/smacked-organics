...
function kitBoxInner(item){
  ...
  return `
    <div class="media-wrap">
      ...
      ${soldOutBanner}
      <button class="wishlist-btn ${wished ? 'active' : ''}" data-id="${item.id}" aria-label="Save to wishlist">
...
function renderKit(){
  ...
  grid.addEventListener('click', (e) => {
    ...
    if(wishBtn){
      ...
      const item = KIT.find(k => k.id === wishBtn.dataset.id);
      const box = grid.querySelector(`.kit-box[data-id="${wishBtn.dataset.id}"]`);
      box.innerHTML = kitBoxInner(item);
      attachTilt(box);
      return;
    }
...
function findItemById(id){
  ...
  return KIT.find(k => k.id === id) || PRODUCTS.find(p => p.id === numericId) || ORGANICS.find(o => o.id === numericId);
}
...
let modalOpenedAt = 0;
function relatedProductsHTML(p, kitItem, organicsItem){
  let pool, colorState;
  if(kitItem){ pool = KIT.filter(x => x.id !== p.id); colorState = kitColors; }
  else if(organicsItem){ pool = ORGANICS.filter(x => x.id !== p.id && x.cat === p.cat); colorState = organicsColors; }
        ...
        ${picks.map(item => {
          ...
          return `
            <div class="related-item" data-id="${item.id}">
              ${iconWrap(content, item.colors[colorState[item.id] || 0])}
...
function openModal(id){
  ...
  card.innerHTML = `
    ...
    <div class="modal-info">
      ...
      <div class="label-row">
        ...
        <button class="size-guide-toggle mono" id="sizeGuideToggle" type="button">size guide</button>
      </div>
      <div class="size-guide" id="sizeGuideTable">
        <table>
          <tr><th></th><th>chest</th><th>length</th><th>sleeve</th></tr>
          ${SIZES.map(s => `<tr><td>${s}</td><td>${SIZE_CHART[s].chest}"</td><td>${SIZE_CHART[s].length}"</td><td>${SIZE_CHART[s].sleeve}"</td></tr>`).join('')}
        </table>
      </div>
      ...
      ${SIZES.some(s => { const st = getStock(p.id, s, color); return st !== null && st <= 0; }) ? `
      ...
      ${relatedProductsHTML(p, kitItem, organicsItem)}
    </div>
  `;
  ...
  document.getElementById('sizeGuideToggle').addEventListener('click', () => {
  ...
  if(notifyToggle){
    ...
    document.getElementById('notifySubmit').addEventListener('click', async () => {
      ...
      const sizeForRequest = selectedSizeBtn ? selectedSizeBtn.dataset.size : SIZES[0];
  ...
  card.querySelectorAll('.related-item').forEach(el => {
    el.addEventListener('click', () => {
      ...
      openModal(isNaN(numericId) || KIT.find(k => k.id === rid) ? rid : numericId);
    });
  });
  ...
  itemsEl.querySelectorAll('button').forEach(b => {
    b.addEventListener('click', () => {
      ...
      if(b.dataset.action === 'remove'){
        toggleWishlist(isNaN(Number(id)) ? id : Number(id));
...
// Mirrors renderNewArrivals above, but pulls from ORGANICS items flagged
// isLimited:true instead of PRODUCTS flagged isNew:true. Sits in its own row
// right below "new arrivals" so both are visible together at the top of the
// homepage without disturbing the rest of the layout.
function renderLimitedDrop(){
  const wrap = document.getElementById('limitedDropWrap');
  const el = document.getElementById('limitedDrop');
  const limitedItems = ORGANICS.filter(p => p.isLimited);
  if(limitedItems.length === 0){ wrap.style.display = 'none'; return; }
  wrap.style.display = '';
  el.innerHTML = limitedItems.map(p => `
    <div class="new-item" data-id="${p.id}">
      <div class="new-badge">limited</div>
      ${iconWrap(mediaContent(p, p.svg), p.colors[0])}
      <div class="new-name">${p.name}</div>
      <div class="new-price mono">${money(p.price)}</div>
    </div>
  `).join('');
...
(async () => {
  ...
  safeRun(renderLimitedDrop, 'renderLimitedDrop');
