...
function openModal(id){
  ...
  const p = kitItem || PRODUCTS.find(x => x.id === id);
  const svg = kitItem ? p.svg : CAT_SVG[p.cat];
  const color = kitItem ? p.colors[kitColors[p.id]] : p.colors[productColors[p.id]];
  selectedSize = null;
  ...
  document.getElementById('addBtn').addEventListener('click', () => {
    ...
    addToCart({...p, color, cat: p.cat || 'kit'}, selectedSize);
    closeOverlay();
    bumpCartIcon();
  });
}
